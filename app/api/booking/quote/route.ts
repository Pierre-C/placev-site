...
export async function POST(request: Request) {
  try {
    ...
    const { date, start, end, companyName, contactName, contactEmail, contactPhone, message } = result.data
    ...
    if (startMin % 30 !== 0 || endMin % 30 !== 0) {
      return NextResponse.json({ error: "Les horaires doivent être à la demi-heure pile (ex: 09:00, 09:30)" }, { status: 422 })
    }

    // 2. Horaires d'ouverture 08:30 - 18:00
    if (startMin < OPEN_FROM || endMin > OPEN_UNTIL) {
      return NextResponse.json({ error: "La salle est disponible de 08h30 à 18h00 uniquement" }, { status: 422 })
    }

    // 3. Fin après début
    if (endMin <= startMin) {
      return NextResponse.json({ error: "L'heure de fin doit être après l'heure de début" }, { status: 422 })
    }

    // 4. Durée minimale 2h
    if (endMin - startMin < MIN_DURATION) {
      return NextResponse.json({ error: "La durée minimale de réservation est de 2 heures" }, { status: 422 })
    }
    ...
    if (!openDays.includes(dayOfWeek)) {
      return NextResponse.json({ error: "Le coworking est fermé à cette date" }, { status: 422 })
    }
    ...
    const indicativePrice = isFullDay ? FULL_DAY_PRICE : Math.round(durationHours * HOURLY_RATE)
